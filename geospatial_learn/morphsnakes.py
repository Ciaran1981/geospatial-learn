...
def morphological_geodesic_active_contour(gimage, iterations,
    ...
    return u
def turbo_pixel_ac(gimage, iterations,
                                          init_level_set='circle', smoothing=1,
                                          threshold='auto', balloon=0,
                                          iter_callback=lambda x: None):
    """Morphological Geodesic Active Contours (MorphGAC).
    ...
    for _ in range(iterations):
        ...
        backgrnd = np.logical_not(u)
        skel = skeletonize(backgrnd)  
...
"""
One of the main goals of morphsnakes is to allow topological changes of the
 contours (ie, allowing merge and split). So, in principle, morphsnakes is not 
 designed to prevent this from happening.

That said, you can easily reproduce this behavior by running 
multiple instances of morphsnakes, once per blob, and find the areas where the
 blobs overlap after each iteration and set them to 0. 
 This code shows the overall idea:
"""
import morphsnakes as ms

def fix_overlapping_levelsets(levelsets):
    # I haven't run this code. It might contain errors
...
#levelsets2 = fix_overlapping_levelsets(levelsets)
for _ in range(10):
    # Evolve each level set a few iterations (1 in this case, it can be also 2 or 3…)
    levelsets = [ms.morphological_geodesic_active_contour(gimg, iterations=1, init_level_set=ls) for ls in levelsets]
    levelsets = fix_overlapping_levelsets(levelsets)
